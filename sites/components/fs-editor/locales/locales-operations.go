...
func (f *CFeature) OpRetakeHandler(r *http.Request, pg feature.Page, ctx context.Context, form context.Context, info *editor.File, eid string) (redirect string) {
	...
	if err := f.LockLocale(eid, info.FSID, info.Code); err != nil {
		...
		f.Editor.PushErrorNotice(eid, printer.Sprintf(`error taking over editing: %[1]s`, err.Error()), true)
		return
	}
	//if info.Code != "" {
...
func (f *CFeature) OpUnlockHandler(r *http.Request, pg feature.Page, ctx context.Context, form context.Context, info *editor.File, eid string) (redirect string) {
	...
	if err := f.UnlockLocales(info.FSID, info.Code); err != nil {
		...
		f.Editor.PushErrorNotice(eid, printer.Sprintf(`error unlocking locale for editing by others: %[1]s`, err.Error()), true)
		return
	}
	if info.Code != "" {
		redirect = f.GetEditorPath() + "/" + info.FSID
...
func (f *CFeature) OpCommitHandler(r *http.Request, pg feature.Page, ctx context.Context, form context.Context, info *editor.File, eid string) (redirect string) {
	...
	if ld, err = f.ReadDraftLocales(info.FSID, info.Code, mountPoints, false); err != nil {
		f.Editor.PushErrorNotice(eid, printer.Sprintf("error reading locales data: %[1]s", err.Error()), true)
		return
	}

	if err = f.performDraftChanges(r, translations, ld); err != nil {
		f.Editor.PushErrorNotice(eid, printer.Sprintf(`error performing draft changes: %[1]s`, err.Error()), true)
		return
	}

	if err = f.WriteDraftLocales(ld, mountPoints); err != nil {
		f.Editor.PushErrorNotice(eid, printer.Sprintf(`error writing draft changes: %[1]s`, err.Error()), true)
		return
	}

	f.Editor.PushInfoNotice(eid, printer.Sprintf(`draft locale changes saved`), true)
...
func (f *CFeature) OpPublishHandler(r *http.Request, pg feature.Page, ctx context.Context, form context.Context, info *editor.File, eid string) (redirect string) {
	...
	f.Editor.PushInfoNotice(eid, printer.Sprintf(`draft locale changes published`), true)
...
func (f *CFeature) OpChangeHandler(r *http.Request, pg feature.Page, ctx context.Context, form context.Context, info *editor.File, eid string) (redirect string) {
	...
	case "copy-translation":
		...
		if parts := strings.Split(changeTarget, "."); len(parts) != 1 {
			f.Editor.PushErrorNotice(eid, printer.Sprintf(`incomplete form submission`), true)
			return
		...
		if parts := strings.Split(dstPath, "/"); len(parts) == 1 {
			...
			f.Editor.PushErrorNotice(eid, printer.Sprintf(`incorrect form submission`), true)
			return
		}
		...
		if dstLd, err = f.ReadDraftLocales(info.FSID, info.Code, dstMountPoints, false); err != nil {
			f.Editor.PushErrorNotice(eid, printer.Sprintf("destination not found, cannot copy"), true)
			return
		}
		...
		if msgs, ok = ld.Data[shasum]; !ok {
			f.Editor.PushErrorNotice(eid, printer.Sprintf(`message not found, cannot copy`), true)
			return
		}
		...
		changed = false
		f.Editor.PushInfoNotice(eid, printer.Sprintf(`translation copied to: %[1]s`, dstPath), true)
	...
	case "add-translation":
		...
		if key, _ = form["add-translation.key"].(string); key == "" {
			f.Editor.PushErrorNotice(eid, printer.Sprintf(`translation key is required to create a new entry`), true)
			return
		} else if comment, _ = form["add-translation.comment"].(string); comment == "" {
			f.Editor.PushErrorNotice(eid, printer.Sprintf(`translation comment is required to create a new entry`), true)
			return
		}
		...
		if _, present := ld.Data[msg.Shasum]; present {
			f.Editor.PushErrorNotice(eid, printer.Sprintf(`translation key exists already`), true)
			return
		}
	...
	case "delete-translation":
		...
		if _, present := ld.Data[shasum]; !present {
			f.Editor.PushErrorNotice(eid, printer.Sprintf(`translation not found, nothing to delete`), true)
			return
		}
	...
	case "add-translation-case":
		...
		if parts := strings.Split(changeTarget, "."); len(parts) != 2 {
			...
			f.Editor.PushErrorNotice(eid, printer.Sprintf(`invalid plural translation case key`), true)
			return
		}
		...
		if msg, ok := ld.Data[shasum][tag]; ok {
			if msg.Translation.Select != nil {
				if _, present := msg.Translation.Select.Cases[key]; present {
					f.Editor.PushErrorNotice(eid, printer.Sprintf(`translation case "%[1]s" exists already`, key), true)
					return
				}
	...
	case "delete-translation-case":
		...
		if parts := strings.Split(changeTarget, "."); len(parts) != 3 {
			...
			f.Editor.PushErrorNotice(eid, printer.Sprintf(`"other" translation case is required, cannot delete`), true)
			return
		}
	...
	case "pluralize-translation":
		...
		if defArg == "" {
			f.Editor.PushErrorNotice(eid, printer.Sprintf(`translation cannot be converted to plural form`), true)
			return
		}
	...
	default:
		f.Editor.PushErrorNotice(eid, printer.Sprintf(`unknown operation`), true)
		return
