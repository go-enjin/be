//go:build driver_db_gorm_postgres || drivers_db || drivers || postgres || all
...
const Tag feature.Tag = "drivers-db-gorm-postgres"
...
type MakeFeature interface {
	...
	//
	//   --db-gorm-postgres-your-tag-uri
	//
	// the environment variable is:
	//
	//   DB_GORM_POSTGRES_YOUR_TAG_URI
	//
...
func (f *CFeature) AddConnection(tag string) MakeFeature {
	...
	if _, exists := f.flags[tag]; exists {
		log.FatalDF(1, "a gorm postgres connection already exists with the given tag: %v", tag)
	}
...
func (f *CFeature) Build(b feature.Buildable) (err error) {
	...
	for tag, flag := range f.flags {
		b.AddFlags(
			&cli.StringFlag{
				Name:    flag,
				Usage:   fmt.Sprintf("db gorm postgres %v connection string", tag),
...
func (f *CFeature) Startup(ctx *cli.Context) (err error) {
	...
	for tag, flag := range f.flags {
		...
		if uri = ctx.String(flag); uri == "" {
			err = fmt.Errorf("db gorm postgres startup error: missing --%v", flag)
			return
		}
		if f.conns[tag], err = gorm.Open(postgres.Open(uri), &gorm.Config{Logger: logger.New(log.Logger(), config)}); err != nil {
			err = fmt.Errorf("db gorm postgres startup error: %v - %v", tag, err)
			return
		}
		log.InfoF("db gorm postgres connected: %v", tag)
	}
	return
}
...
func (f *CFeature) Shutdown() {
	for tag, conn := range f.conns {
		if db, err := conn.DB(); err != nil {
			log.DebugF("error getting db gorm postgres: %v - %v", tag, err)
		} else {
			if err = db.Close(); err != nil {
				log.ErrorF("error closing db gorm postgres: %v - %v", tag, err)
			} else {
				log.InfoF("closed db gorm postgres: %v", tag)
			}
		}
	}
}
...
func (f *CFeature) DB(tag string) (db interface{}, err error) {
	...
	err = fmt.Errorf("db gorm postgres connection %v not found", tag)
	return
}
...
func (f *CFeature) MustDB(tag string) (db interface{}) {
	...
	log.FatalDF(1, "db gorm postgres connection %v not found", tag)
	return
}