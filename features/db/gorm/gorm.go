...
const (
	Tag feature.Tag = "db-gorm"

	gTypeFlagFormat string = string(Tag) + "-%v-type"
	gUriFlagFormat  string = string(Tag) + "-%v-uri"
)
...
func (f *CFeature) AddConnection(tag string) MakeFeature {
	...
	f.flags[tag] = append(f.flags[tag],
		fmt.Sprintf(gTypeFlagFormat, tag),
		fmt.Sprintf(gUriFlagFormat, tag),
	)
	return f
}
