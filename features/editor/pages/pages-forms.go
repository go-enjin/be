...
func (f *CFeature) ParseFormToDraft(fields context.Fields, form context.Context, info *editor.File, r *http.Request) (pm *matter.PageMatter, redirect string, errs map[string]error) {
	...
	if contents, ok := form.FirstString("body"); ok {
		...
		f.Editor.PushErrorNotice(eid, printer.Sprintf(`error decoding form body: "%v"`, err), true)
		redirect = f.SelfEditor().GetEditorPath() + "/" + info.EditFilePath()
		return
	}
...
func (f *CFeature) ParseCreatePageForm(r *http.Request, pg feature.Page, ctx, form context.Context, info *editor.File, eid string, redirect *string) (dstUri, dstArchetype string, dstInfo *editor.File, dstFS feature.FileSystemFeature, dstMP *feature.CMountPoint, dstExists bool, stop bool) {
	...
	if fileLang, _ = form.FirstString(editor.CreatePageActionKey + "~dst-lang"); fileLang == "" {
		f.Editor.PushWarnNotice(eid, printer.Sprintf(`a locale is required to create a new page`), true)
		stop = true
		return
	} else if fileLocale, err = language.Parse(fileLang); err != nil {
		f.Editor.PushWarnNotice(eid, printer.Sprintf(`a valid locale is required to create a new page`), true)
		stop = true
		return
	...
	if fileFormat, _ = form.FirstString(editor.CreatePageActionKey + "~dst-format"); fileFormat == "" {
		f.Editor.PushWarnNotice(eid, printer.Sprintf(`a file format is required to create a new page`), true)
		*redirect = f.SelfEditor().GetEditorPath() + "/" + info.EditParentDirectoryPath()
		stop = true
		return
	} else if _, matched := t.MatchFormat("not-a-thing." + fileFormat); matched == "" {
		f.Editor.PushWarnNotice(eid, printer.Sprintf(`a valid file format is required to create a new page`), true)
		*redirect = f.SelfEditor().GetEditorPath() + "/" + info.EditParentDirectoryPath()
		stop = true
		return
	} else {
	...
	if createMode == "arch" {
		if archetype, _ = form.FirstString(editor.CreatePageActionKey + "~dst-archetype"); archetype != "" && !slices.Within(archetype, archetypes) {
			...
			f.Editor.PushWarnNotice(eid, printer.Sprintf(`a supported archetype is required to create a new page, supported archetypes are: %[1]s`, archetypeNames), true)
			*redirect = f.SelfEditor().GetEditorPath() + "/" + info.EditParentDirectoryPath()
			stop = true
			return
		} else {
			if archetype != "" {
				...
				if _, match = t.MatchFormat(archetype); match == "" {
					f.Editor.PushWarnNotice(eid, printer.Sprintf(`archetype page format not supported`), true)
					*redirect = f.SelfEditor().GetEditorPath() + "/" + info.EditParentDirectoryPath()
					return
				}
				fileFormat = match
