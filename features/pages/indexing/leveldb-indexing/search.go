...
func (f *CFeature) indexStubPage(stub *pagecache.Stub, p *page.Page) (err error) {
	...
	if err = f.leveldb.Put([]byte(fmt.Sprintf("stub:%v:shasum", stub.Shasum)), []byte(stubIdxStr), nil); err != nil {
		log.Error(err)
		return
	}
	if err = f.leveldb.Put([]byte(fmt.Sprintf("stub:%d:index", stubIdx)), []byte(stub.Shasum), nil); err != nil {
		log.Error(err)
		return
	}

	update := func(content string) (err error) {
		words := regexps.RxKeywords.FindAllString(content, -1)
		for _, word := range words {
			word = strings.ToLower(word)
			key := "contents:" + word
			list, _ := f.leveldb.Get([]byte(key), nil)
			sids := strings.Split(string(list), ",")
			if !beStrings.StringInSlices(stubIdxStr, sids) {
				sids = append(sids, stubIdxStr)
			}
			if err = f.leveldb.Put([]byte(key), []byte(strings.Join(sids, ",")), nil); err != nil {
