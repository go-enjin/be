...
func (f *CFeature) ServeRevokePage(settingsPath string, saf feature.SiteAuthFeature, w http.ResponseWriter, r *http.Request) (handled bool, redirect string) {
	...
	t := f.Site().SiteTheme()
	if err = f.Site().PrepareAndServePage("site-auth", "backup-codes--revoke", r.URL.Path, t, w, r, ctx); err != nil {
		log.ErrorRF(r, "error preparing and serving backup-codes--revoke page: %v", err)
		panic(err)
	}
	handled = true
	return
