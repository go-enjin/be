...
func (f *CFeature) hasSecureProvision(key string, r *http.Request) (present bool) {
	...
	} else if secret := provisions.String(key, ""); secret != "" {
		present = true
		return
	}
	return
...
func (f *CFeature) getSecureProvision(key string, r *http.Request) (secret string, err error) {
	var secure, provisions context.Context
	if secure, err = f.getSecureContext(r); err != nil {
		return
	} else if provisions = secure.Context("provisions"); provisions == nil {
...
func (f *CFeature) setSecureProvision(key, secret string, r *http.Request) (err error) {
	f.userLock(r)
	defer f.userUnlock(r)
	var secure, provisions context.Context
	...
	provisions.SetSpecific(key, secret)
