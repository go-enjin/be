...
func (f *CFeature) OpChangeHandler(r *http.Request, pg feature.Page, ctx, form context.Context, info *editor.File, eid string) (redirect string) {
	...
	case "~add-new":
		...
		if addNewKeyName == "" {
			f.Editor.Site().PushErrorNotice(eid, true, printer.Sprintf("kebab-cased key name is required"))
		} else {
			...
			if !AreVariablesAllowed(kvName, pg.Format(), fields) {
				...
				f.Editor.Site().PushErrorNotice(eid, true, printer.Sprintf(
			...
			if errMsg != "" {
				changeType = "nop"
				f.Editor.Site().PushErrorNotice(eid, true, errMsg)
			}
	...
	if err = f.WriteDraftPage(info, pm); err != nil {
		f.Editor.Site().PushErrorNotice(eid, true, printer.Sprintf("error saving draft page changes: \"%[1]s\"", err.Error()))
		return
	}
...
func (f *CFeature) OpFileCommitHandler(r *http.Request, pg feature.Page, ctx, form context.Context, info *editor.File, eid string) (redirect string) {
	...
	if len(errs) > 0 {
		...
		f.Editor.Site().PushInfoNotice(eid, true, printer.Sprintf("%[1]s draft page changes saved.", info.File))
	}
...
func (f *CFeature) OpFilePublishHandler(r *http.Request, pg feature.Page, ctx, form context.Context, info *editor.File, eid string) (redirect string) {
	...
	if _, hasMatter := form["matter"]; hasMatter {
		if _, hasBody := form["body"]; hasBody {
			...
			if pm, redirect, errs = f.ParseFormToDraft(pm, fields, form, info, r); redirect != "" {
				...
				f.Editor.Site().PushErrorNotice(eid, true, printer.Sprintf("error writing draft page: \"%[1]s\"", err.Error()))
				return
			}
		}
	}
	...
	if err = f.PublishDraftPage(info); err != nil {
		f.Editor.Site().PushErrorNotice(eid, true, printer.Sprintf("error publishing draft page: \"%[1]s\"", err.Error()))
		return
	} else if err = f.SelfEditor().RemoveDraft(info); err != nil {
		f.Editor.Site().PushErrorNotice(eid, true, printer.Sprintf("error removing final draft page: \"%[1]s\"", err.Error()))
		return
	} else if err = f.UnLockEditorFile(info.FSID, info.FilePath()); err != nil {
		f.Editor.Site().PushErrorNotice(eid, true, printer.Sprintf("error unlocking page file: \"%[1]s\"", err.Error()))
		return
	}

	f.Editor.Site().PushInfoNotice(eid, true, printer.Sprintf("%[1]s draft page changes published.", info.File))
...
func (f *CFeature) OpFileDeleteHandler(r *http.Request, pg feature.Page, ctx, form context.Context, info *editor.File, eid string) (redirect string) {
	...
	default:
		...
		if pm, err = f.ReadPageMatter(info); err != nil {
			...
			f.Editor.Site().PushErrorNotice(eid, true, printer.Sprintf("error removing file: %[1]s - %[2]s", info.Name, err.Error()))
			return
		}
...
func (f *CFeature) OpFileIndexHandler(r *http.Request, pg feature.Page, ctx, form context.Context, info *editor.File, eid string) (redirect string) {
	...
	if lockedBy, locked := f.IsEditorFileLocked(info.FSID, info.FilePath()); locked && eid != lockedBy {
		f.Editor.Site().PushErrorNotice(eid, true, printer.Sprintf("Cannot index, file is locked by another user"))
		return
	}

	if _, _, err := f.InfoRenderCheck(info); err != nil {
		f.Editor.Site().PushErrorNotice(eid, true, printer.Sprintf(`cannot re-index broken pages`))
		f.Editor.Site().PushErrorNotice(eid, true, printer.Sprintf(`page format render error: %[1]s`, err.Error()))
		return
	}
	...
	f.Editor.Site().PushInfoNotice(eid, true, printer.Sprintf(`"%[1]s" has been added to page indexing`, info.Name))
...
func (f *CFeature) OpFileDeIndexHandler(r *http.Request, pg feature.Page, ctx, form context.Context, info *editor.File, eid string) (redirect string) {
	...
	if lockedBy, locked := f.IsEditorFileLocked(info.FSID, info.FilePath()); locked && eid != lockedBy {
		f.Editor.Site().PushErrorNotice(eid, true, printer.Sprintf("Cannot remove index, file is locked by another user"))
		return
	}
	...
	f.Editor.Site().PushInfoNotice(eid, true, printer.Sprintf(`"%[1]s" has been removed from page indexing`, info.Name))
...
func (f *CFeature) OpFileTranslateHandler(r *http.Request, pg feature.Page, ctx, form context.Context, info *editor.File, eid string) (redirect string) {
	...
	if _, _, err := f.InfoRenderCheck(info); err != nil {
		f.Editor.Site().PushErrorNotice(eid, true, printer.Sprintf(`cannot translate broken pages`))
		f.Editor.Site().PushErrorNotice(eid, true, printer.Sprintf(`page format render error: %[1]s`, err.Error()))
		return
	}
	...
	if _, srcShasum, created, updated, err = srcMP.ROFS.FileStats(info.FilePath()); err != nil {
		...
		f.Editor.Site().PushErrorNotice(eid, true, printer.Sprintf(`error reading "%[1]s" page matter: %[2]s`, srcUri, err.Error()))
		return
	}
	srcMatter.Matter.SetSpecific("translates", info.Url())
	if srcData, err = srcMatter.Bytes(); err != nil {
		f.Editor.Site().PushErrorNotice(eid, true, printer.Sprintf(`error preparing "%[1]s" page matter: %[2]s`, srcUri, err.Error()))
		return
		...
		f.Editor.Site().PushErrorNotice(eid, true, printer.Sprintf(`source and destination file shasums are the same`))
	...
	f.Editor.Site().PushWarnNotice(eid, true, printer.Sprintf(`"%[1]s" %[2]s page started, please translate and publish`, info.Name, dstInfo.Locale.String()))
