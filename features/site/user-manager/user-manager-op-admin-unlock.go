...
func (f *CFeature) opAdminUnlockUser(form context.Context, r *http.Request) {
	eid := userbase.GetCurrentEID(r)
	printer := lang.GetPrinterFromRequest(r)

	if !userbase.CurrentUserCan(r, f.Action("update", "user")) {
		log.WarnRF(r, "user %q attempted to admin-unlock a new user without permission!", eid)
	...
	if userEID = form.String("target", ""); userEID == "" {
		return
	} else if confirmed = form.String(editor.AdminUnlockUserActionKey+"-confirmed", "false"); confirmed != "true" {
		return
	}
	...
	if !su.UserPresent(userEID) {
		log.WarnRF(r, "user %q attempting to admin-unlock a user that does not exist!", eid)
		return
	}

	if err := su.SetUserAdminLocked(r, userEID, false); err != nil {
