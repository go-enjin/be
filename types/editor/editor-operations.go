...
func (f *CEditorFeature[MakeTypedFeature]) OpFileUnlockHandler(r *http.Request, pg feature.Page, ctx, form beContext.Context, info *editor.File, eid string) (redirect string) {
	...
	if info.Locked {
		f.Editor.PushErrorNotice(eid, cannotUnlockErr, true)
		return
	...
	f.Editor.PushInfoNotice(eid, printer.Sprintf("%[1]s unlocked for others to edit.", info.File), true)
...
func (f *CEditorFeature[MakeTypedFeature]) OpFileRetakeHandler(r *http.Request, pg feature.Page, ctx, form beContext.Context, info *editor.File, eid string) (redirect string) {
	...
	if err = f.LockEditorFile(eid, info.FSID, info.FilePath()); err != nil {
		f.Editor.PushErrorNotice(eid, printer.Sprintf("error locking file for editing: \"%[1]s\"", err.Error()), true)
		return
	}
	f.Editor.PushInfoNotice(eid, printer.Sprintf("%[1]s editing taken over.", info.File), true)
...
func (f *CEditorFeature[MakeTypedFeature]) OpPathDeleteHandler(r *http.Request, pg feature.Page, ctx, form beContext.Context, info *editor.File, eid string) (redirect string) {
	...
	f.Editor.PushInfoNotice(eid, printer.Sprintf("%[1]s directory deleted.", info.Name), true)
...
func (f *CEditorFeature[MakeTypedFeature]) OpFileCommitHandler(r *http.Request, pg feature.Page, ctx, form beContext.Context, info *editor.File, eid string) (redirect string) {
	...
	if err := f.SelfEditor().WriteDraft(info, []byte(body)); err != nil {
		f.Editor.PushErrorNotice(eid, printer.Sprintf("error saving to draft file: \"%[1]s\"", err.Error()), true)
		return
	}
	...
	f.Editor.PushInfoNotice(eid, printer.Sprintf("%[1]s draft changes saved.", info.File), true)
...
func (f *CEditorFeature[MakeTypedFeature]) OpFilePublishHandler(r *http.Request, pg feature.Page, ctx, form beContext.Context, info *editor.File, eid string) (redirect string) {
	...
	if body, present := form["body"].(string); present {
		body = strings.ReplaceAll(body, "\r", "")
		if err = f.SelfEditor().WriteDraft(info, []byte(body)); err != nil {
			f.Editor.PushErrorNotice(eid, printer.Sprintf("error saving final changes to draft: \"%[1]s\"", err.Error()), true)
			return
		}
		ctx.SetSpecific("Body", body)
	}
	...
	if data, err = f.SelfEditor().ReadDraft(info); err != nil {
		f.Editor.PushErrorNotice(eid, printer.Sprintf("error reading final draft: \"%[1]s\"", err.Error()), true)
		return
	} else if err = f.SelfEditor().WriteFile(info, data); err != nil {
		f.Editor.PushErrorNotice(eid, printer.Sprintf("error writing file: \"%[1]s\"", err.Error()), true)
		return
	} else if err = f.SelfEditor().RemoveDraft(info); err != nil {
		f.Editor.PushErrorNotice(eid, printer.Sprintf("error removing final draft: \"%[1]s\"", err.Error()), true)
		return
	} else if err = f.SelfEditor().UnLockEditorFile(info.FSID, info.FilePath()); err != nil {
		f.Editor.PushErrorNotice(eid, printer.Sprintf("error unlocking file: \"%[1]s\"", err.Error()), true)
		return
	}

	f.Editor.PushInfoNotice(eid, printer.Sprintf("%[1]s draft changes published.", info.File), true)
	redirect = f.SelfEditor().GetEditorPath() + "/" + info.EditDirectoryPath()
...
func (f *CEditorFeature[MakeTypedFeature]) OpFileMoveHandler(r *http.Request, pg feature.Page, ctx, form beContext.Context, info *editor.File, eid string) (redirect string) {
	...
	if !srcExists {
		...
		f.Editor.PushErrorNotice(eid, printer.Sprintf(`cannot move "%[2]s" to "%[1]s": filesystem not found`, dstInfo.FSID, srcUri), true)
		return
	} else if dstMP.RWFS == nil {
		f.Editor.PushErrorNotice(eid, printer.Sprintf(`cannot move "%[2]s" to "%[1]s": filesystem is read-only`, dstInfo.FSID, srcUri), true)
		return
	} else if srcFS == nil || srcMP == nil {
		f.Editor.PushErrorNotice(eid, printer.Sprintf(`cannot move "%[2]s" from "%[1]s": filesystem not found`, info.FSID, srcUri), true)
		return
	} else if srcMP.RWFS == nil {
		f.Editor.PushErrorNotice(eid, printer.Sprintf(`cannot move "%[2]s" from "%[1]s": filesystem is read-only`, info.FSID, srcUri), true)
		return
	}
	...
	if _, srcShasum, created, updated, err = srcMP.RWFS.FileStats(info.FilePath()); err != nil {
		...
		f.Editor.PushErrorNotice(eid, printer.Sprintf(`error reading "%[1]s": %[2]s`, srcUri, err.Error()), true)
		return
		...
		f.Editor.PushErrorNotice(eid, printer.Sprintf(`source and destination file shasums differ`), true)
		return
	} else if err = f.UnLockEditorFile(info.FSID, info.FilePath()); err != nil {
		f.Editor.PushErrorNotice(eid, printer.Sprintf(`error unlocking source file "%[1]s" before deleting during move: %[2]s`, srcUri, err.Error()), true)
		return
	} else if err = srcMP.RWFS.Remove(info.FilePath()); err != nil {
		f.Editor.PushErrorNotice(eid, printer.Sprintf(`error removing "%[1]s": %[2]s`, srcUri, err.Error()), true)
		return
	}

	f.Editor.PushInfoNotice(eid, printer.Sprintf(`moved "%[1]s" to "%[2]s"`, srcUri, dstUri), true)
...
func (f *CEditorFeature[MakeTypedFeature]) OpFileCopyHandler(r *http.Request, pg feature.Page, ctx, form beContext.Context, info *editor.File, eid string) (redirect string) {
	...
	f.Editor.PushInfoNotice(eid, printer.Sprintf(`copied "%[1]s" to "%[2]s"`, srcUri, dstUri), true)
	if v, _ := form["return"].(string); v == "directory" {
		redirect = f.SelfEditor().GetEditorPath() + "/" + dstInfo.EditDirectoryPath()
	} else {
		redirect = f.SelfEditor().GetEditorPath() + "/" + dstInfo.EditFilePath()
...
func (f *CEditorFeature[MakeTypedFeature]) OpFileTranslateHandler(r *http.Request, pg feature.Page, ctx, form beContext.Context, info *editor.File, eid string) (redirect string) {
	...
	f.Editor.PushInfoNotice(eid, printer.Sprintf(`"%[1]s" %[2]s translation started`, info.Name, dstInfo.Locale.String()), true)
